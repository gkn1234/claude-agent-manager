...
| [deployment-architecture](architecture/deployment-architecture.md) | 生产部署架构：systemd 管理的 Next.js standalone 模式、环境初始化脚本（OS 检测、Node.js 24、pnpm、Claude CLI）、EnvironmentFile 环境变量注入、deploy.sh 部署流水线（git pull -> build -> restart）、CLI 热更新。 |
...
| [deployment-operations](guides/deployment-operations.md) | 部署和运维操作指南：环境初始化（setup-ec2.sh）、.env 配置、首次部署和更新（deploy.sh）、Claude CLI 更新、systemctl 服务管理、数据库备份。 |
