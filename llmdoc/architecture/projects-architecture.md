...
- `src/lib/utils/git.ts`（`isGitRepo`、`gitClone`、`gitInit`、`getGitRemote`、`ensureGitignoreEntry`）：Git CLI 工具函数（无 shell 注入风险）。`gitClone` 为异步函数（`promisify(execFile)`），带 120 秒超时和失败自动清理；其余函数仍使用 `execFileSync`。
...
- **3. 模式专属 git 操作：**
  ...
  - **new** (`src/app/api/projects/route.ts:64-75`)：用 `mkdirSync` 创建目录，然后执行 `gitInit(finalDir)`。目录不得已存在（409）。
