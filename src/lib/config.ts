...
const CONFIG_DEFAULTS: Record<string, string> = {
  ...
  poll_interval: '5',
  research_prompt: `永远用中文回复。

你正在一个任务派发系统中工作。初始化已完成，请在当前工作区中进行需求调研和分析：
...
3. 分支命名：请根据项目命名规范和任务描述自主决定分支名称。建议使用 feat-、fix-、chore- 等前缀加简短描述的格式（如 feat-add-user-auth，务必不要有 / 符号），保持简洁、有意义、符合团队惯例
4. 理解项目结构
